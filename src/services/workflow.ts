...
export const getWorkflowsWithPage = (req: GetWorkflowsWithPageRequest) => {
  return request.get<GetWorkflowsWithPageResponse>(`/workflows`, { params: req })
}

// 在当前账号下新增工作流
export const createWorkflow = (req: CreateWorkflowRequest) => {
  return request.post<BaseResponse<{ id: string }>>(`/workflows`, req)
}

// 修改工作流基础信息
export const updateWorkflow = (workflow_id: string, req: UpdateWorkflowRequest) => {
  return request.post<BaseResponse<unknown>>(`/workflows/${workflow_id}`, req)
}

// 获取工作流基础信息
export const getWorkflow = (workflow_id: string) => {
  return request.get<GetWorkflowResponse>(`/workflows/${workflow_id}`)
}

// 删除指定的工作流
export const deleteWorkflow = (workflow_id: string) => {
  return request.post<BaseResponse<unknown>>(`/workflows/${workflow_id}/delete`)
}
...
export const publishWorkflow = (workflow_id: string) => {
  return request.post<BaseResponse<unknown>>(`/workflows/${workflow_id}/publish`)
}

// 取消发布指定的工作流
export const cancelPublishWorkflow = (workflow_id: string) => {
  return request.post<BaseResponse<unknown>>(`/workflows/${workflow_id}/cancel-publish`)
}
