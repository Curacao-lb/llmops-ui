...
export const useDeleteDebugConversation = () => {
  ...
  const handleDeleteDebugConversation = async (app_id: string, callback?: () => void) => {
    Modal.warning({
      title: '要清空调试会话记录吗?',
      content: '清空后将无法恢复该应用的调试会话历史记录。',
      hideCancel: false,
      onOk: async () => {
        try {
          loading.value = true
...
export const useGetDebugConversationMessagesWithPage = () => {
  ...
  return { loading, messages, paginator, created_at, loadDebugConversationMessages }
}
...
export const useGetDebugConversationSummary = () => {
  ...
  const loadDebugConversationSummary = async (app_id: string) => {
    try {
      ...
      debug_conversation_summary.value = resp.data.summary
    } finally {
      loading.value = false
    }
  }
