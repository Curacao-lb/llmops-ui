...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between">
      <div class="text-lg font-bold text-gray-700">{{ api_key_id ? '更新' : '新增' }}密钥</div>
      <a-button
        type="text"
        class="!text-gray-700"
        size="small"
    ...
    <div class="pt-6">
      <a-form ref="formRef" :model="form" layout="vertical" @submit="saveApiKey">
        <a-form-item field="is_active" label="密钥状态">
          <a-switch v-model:model-value="form.is_active" />
        </a-form-item>
        <a-form-item field="remark" label="密钥备注">
          <a-textarea
            v-model:model-value="form.remark"
            :max-length="100"
            show-word-limit
            placeholder="请输入密钥备注，用于描述密钥的基础信息"
          />
        </a-form-item>
        <!-- 底部按钮 -->
        <div class="flex items-center justify-between">
