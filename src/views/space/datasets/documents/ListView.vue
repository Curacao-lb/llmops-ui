<script setup lang="ts">
  ...
  // 切换文档启用状态，成功后同步本地列表
  const onToggleEnabled = (document_id: string, rowIndex: number, value: boolean) => {
    handleUpdateEnabled(String(route.params?.dataset_id), document_id, value, () => {
      const row = documents.value[rowIndex]
      if (row) row.enabled = value
    })
  }
</script>
            ...
            <template #cell="{ record, rowIndex }">
              <a-space :size="0">
                ...
                <a-switch
                  ...
                  @change="(value: any) => onToggleEnabled(record.id, rowIndex, value)"
                />
