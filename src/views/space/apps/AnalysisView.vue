          ...
          acc[field] = {
            ...
            xAxis: {
              ...
              data: app_analysis.value[field]?.x_axis?.map((value: number) => {
                return dayjs(value * 1000).format('MMM D, YYYY')
              }),
              splitLine: {
                show: true,
                lineStyle: { color: '#eeeeee', width: 1, type: 'solid' },
              },
            },
            // y轴数据为数值
            yAxis: { type: 'value' },
...
<template>
  <div class="px-6 py-5 h-full overflow-auto scrollbar-w-none">
    <!-- 概览指标 -->
    <div class="flex flex-col gap-5 mb-5">
      ...
      <!-- 指标卡片 -->
      <a-spin :loading="getAppAnalysisLoading" class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="flex flex-col bg-white rounded-lg border border-gray-200 p-4 h-[300px]">
            ...
            <div class="w-full flex-1">
              <v-chart :option="trendOption?.total_messages_trend" :autoresize="true" />
            </div>
          </div>
          <div class="flex flex-col bg-white rounded-lg border border-gray-200 p-4 h-[300px]">
            <!-- 标题 -->
            ...
            <!-- 可视化图表 -->
            <div class="w-full flex-1">
              <v-chart :option="trendOption?.active_accounts_trend" :autoresize="true" />
            </div>
          </div>
          <div class="flex flex-col bg-white rounded-lg border border-gray-200 p-4 h-[300px]">
            <!-- 标题 -->
            ...
            <!-- 可视化图表 -->
            <div class="w-full flex-1">
              <v-chart :option="trendOption?.avg_of_conversation_messages_trend" :autoresize="true" />
            </div>
          </div>
          <div class="flex flex-col bg-white rounded-lg border border-gray-200 p-4 h-[300px]">
            <!-- 标题 -->
            ...
            <!-- 可视化图表 -->
            <div class="w-full flex-1">
              <v-chart :option="trendOption?.cost_consumption_trend" :autoresize="true" />
            </div>
          </div>
        </div>
      </a-spin>
